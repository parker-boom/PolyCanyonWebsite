 ...
 * changing source photographs. Originals are kept.
 */
...
for (const number of [24, 7, 16, 6, 10, 31])
  for (const width of [800, 1600])
    jobs.push({
      source: path.join(sourceRoot, 'main', `M-${number}.webp`),
      target: path.join(outputRoot, 'home', `M-${number}-${width}.webp`),
      width,
      quality: 82,
    });
