...
if (process.argv.includes('--check')) {
  try {
    ...
    console.log(
      `Verified ${jobs.length} generated images, originals and responsive manifests.`
    );
  } catch (error) {
    console.error(
      'Missing or stale generated media. Run npm run media:generate and include the generated files.'
    );
  ...
  async function worker() {
    for (const { source, target, width, quality = 80 } of queue) {
      ...
      dimensions[path.relative(outputRoot, target).split(path.sep).join('/')] =
        info.width;
      heights[path.relative(outputRoot, target).split(path.sep).join('/')] =
        info.height;
    }
  }
  ...
  const pairs = Object.entries(dimensions)
    .flatMap(([file, width]) => {
      ...
      return smallerWidth ? [[file, smallerFile, width, smallerWidth]] : [];
    })
  ...
  const rows = pairs.map(
    ([file, , width, smallerWidth], index) =>
      `  [full${index}, small${index}, ${width}, ${smallerWidth}, ${heights[file]}],`
  );
  await writeFile(
