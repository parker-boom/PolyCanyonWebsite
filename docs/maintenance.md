# Content, maintenance, and releases
...
node scripts/check-map-directions.mjs
```

The map check verifies destination labeling before and after loading, opt-in behavior, walking-direction URL parameters, and written steps on mobile and desktop. It stubs the third-party iframe; it does not claim to test Google’s route calculation.
...
2. Run the browser regressions above against that preview and visually review desktop/mobile archive, Home, About, App, and photo viewing. Browser automation is an optional developer tool, not a runtime dependency of this site.
...
The main routes are `/`, `/structures`, `/about` and `/app`. Old Info, Map and Download links redirect to About’s visiting section or App. Structure slugs, `/support` and `/privacy` remain stable. One responsive presentation handles each page; there are no separate mobile layouts.

The media generator records each display image’s intrinsic width and height alongside responsive sources, so portrait and landscape photographs reserve their correct frame before loading. Original images and research are unchanged. Search and sorting still use URL parameters; the shared route-position and detail hooks preserve archive return position. Photo viewing supports keyboard arrows, Escape, zoom buttons, pointer pinch/pan and horizontal swipes.

Run `node scripts/check-map-directions.mjs` with the same Playwright setup to verify destination labeling, opt-in embedding and supported external walking URLs. Written walking steps remain visible together. The preview keeps map providers and source documents optional; it adds no telemetry, service, or API key.
