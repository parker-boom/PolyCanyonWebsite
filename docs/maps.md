...
On About, the embedded map marks the Entry Arch. The walking-directions link opens Google Maps with campus as the origin, the Entry Arch as the destination, and walking selected. Written directions remain on the page even if Google is unavailable. Individual structure pages continue to show a destination pin and a location link.
...
[Google Maps URLs](https://developers.google.com/maps/documentation/urls/get-started#directions-action) support external walking directions without an API key, using `api=1`, `origin`, `destination`, and `travelmode=walking`. Keeping that link makes directions available without adding account configuration to this site. The tradeoff is that visitors open Google Maps to see the walking route. Both the About copy and the map caption state this explicitly, before and after the destination map loads.
